...
export const SIGN_IN = (form_data) => async (dispatch) => {
  try {
    dispatch({ type: SET_LOADING_AUTH });
...
export const FORGOT_PASSWORD = (form_data) => async (dispatch) => {
  try {
    dispatch({ type: SET_LOADING_AUTH });
    delete axios.defaults.headers.common["Authorization"];

    const res = await axios.post(`${url}/auth/forgot-password`, form_data);
    console.error(res);
...
export const RESET_PASSWORD = (form_data) => async (dispatch) => {
  try {
    delete axios.defaults.headers.common["Authorization"];

    dispatch({ type: SET_LOADING_AUTH });
    // console.log(form_data);
    const { password, passwordConfirmation, code } = form_data;
